    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 items-end">
        ...
        <div className="flex-1">
          <label className="block text-[9px] uppercase font-bold text-zinc-400 tracking-wider mb-1 truncate">{translations.block_btn_label}</label>
          <input
            type="text"
            value={button.label}
            onChange={(e) => updateFocusedBlock(b => ({ buttonContent: { ...b.buttonContent!, label: e.target.value } }))}
      ...
      <div className="space-y-1 mt-1 bg-zinc-900/10 p-2 rounded-xl border border-zinc-900">
        <div className="flex justify-between items-center text-[8px] font-bold text-zinc-500 uppercase">
          <span>{lang === 'en' ? 'Button Font Size' : 'Размер шрифта кнопки'}</span>
          <span className="font-mono text-zinc-400">
            {focusedBlock.customTitleFontSize !== undefined ? focusedBlock.customTitleFontSize : 14}px
          </span>
        </div>
        <input
          type="range"
          min="12"
          max="100"
          value={focusedBlock.customTitleFontSize !== undefined ? focusedBlock.customTitleFontSize : 14}
      ...
      <div>
        ...
        <input
          type="text"
          value={button.url}
          onChange={(e) => updateFocusedBlock(b => ({ buttonContent: { ...b.buttonContent!, url: e.target.value } }))}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => updateFocusedBlock(b => ({ buttonContent: { ...b.buttonContent!, variant: e.target.value as any } }))}
          className="w-full bg-zinc-900 border border-zinc-800 text-xs rounded-lg p-2 text-white cursor-pointer"
        >
          <option value="primary">Bold Solid High Contrast</option>
          <option value="secondary">Outline Subtle Elegance</option>
        </select>
      </div>
    </div>
  );
