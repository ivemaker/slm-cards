...
export const DishInspector: React.FC<DishInspectorProps> = ({
  focusedBlock,
  lang,
  updateFocusedBlock,
  handleImageUpload,
  ...
  const item = focusedBlock.dishContent!;
  ...
  const handleUploadSlot = async (file: File, index: number) => {
    ...
    try {
      ...
      updateFocusedBlock(b => {
        const d = b.dishContent!;
        const currentImages = d.images && d.images.length > 0 ? [...d.images] : [d.image || '/misty_rainy_forest_1781780284992.jpg'];
        ...
        return {
          dishContent: {
            ...
            image: index === 0 ? base64 : (d.image || base64)
          }
        };
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-[10px] font-black uppercase text-orange-500 tracking-wider flex items-center gap-1.5 mt-1.5">
        ...
        <span>{lang === 'en' ? 'Dish Details' : 'ДЕТАЛИ БЛЮДА'}</span>
      </div>
      ...
      <div className="flex gap-2 items-end">
        ...
        <div className="flex-1">
          <label className="block text-[9px] uppercase font-bold text-zinc-400 tracking-wider mb-1 truncate">
            {lang === 'en' ? 'Dish Name' : 'Название блюда'}
          </label>
          <input
            type="text"
            value={item.name}
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, name: e.target.value } }))}
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, buttonText: e.target.value } }))}
          placeholder={lang === 'en' ? '+ Add to Cart' : '+ В корзину'}
          className="w-full bg-zinc-900 border border-zinc-800 text-xs rounded-lg p-2 text-white outline-none"
        />
      </div>

      {/* Grid: Price and Weight */}
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            step="0.01"
            value={item.price}
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, price: parseFloat(e.target.value) || 0 } }))}
        ...
        <div>
          <label className="block text-[9px] uppercase font-bold text-zinc-400 tracking-wider mb-1">
            {lang === 'en' ? 'Weight' : 'Вес / Порция'}
          </label>
          <input
            type="text"
            value={item.weight || ''}
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, weight: e.target.value } }))}
            className="w-full bg-zinc-900 border border-zinc-800 text-xs rounded-lg p-2 text-white outline-none"
            placeholder={lang === 'en' ? 'e.g. 250g' : 'например, 250 г'}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 bg-zinc-900/40 p-2.5 rounded-xl border border-zinc-800/60">
        <div>
          ...
          <select
            ...
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, imageLayout: e.target.value as any } }))}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, imageSize: e.target.value as any } }))}
            className="w-full bg-zinc-900 border border-zinc-800 text-xs rounded-lg p-2 text-white focus:outline-none cursor-pointer"
          >
      ...
      {/* SECTION: PRICE & WEIGHT STYLE */}
      <div className="pt-3 border-t border-zinc-800/60" />
      <div className="text-[10px] font-black uppercase text-orange-500 tracking-wider flex items-center gap-1.5">
        <span className="w-1.5 h-3.5 bg-orange-500 rounded-full" />
        <span>{lang === 'en' ? 'Price & Weight Styling' : 'СТИЛИЗАЦИЯ ЦЕНЫ И ВЕСА'}</span>
      </div>
      ...
      <div className="bg-zinc-900/40 p-2.5 rounded-xl border border-zinc-800/60 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, priceSize: parseInt(e.target.value) } }))}
            className="w-full accent-white bg-zinc-950 h-1.5 rounded-lg cursor-pointer"
          />
          ...
          <div className="flex gap-2 items-center pt-1">
            <div className="flex-1">
              ...
              <div className="relative w-full h-8 rounded-lg border border-zinc-800 flex items-center justify-center bg-zinc-900 cursor-pointer hover:border-zinc-700 transition">
                <input
                  ...
                  onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, priceColor: e.target.value } }))}
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                />
                <div 
                  className="w-4 h-4 rounded border border-zinc-700 shadow-inner"
            ...
            <button
              type="button"
              onClick={() => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, priceColor: '#ffffff' } }))}
        ...
        <div className="space-y-1.5 pt-2 border-t border-zinc-800/30">
          <div className="flex justify-between items-center text-[9px] uppercase font-bold text-zinc-400 tracking-wider">
            <span>{lang === 'en' ? 'Weight Font Size' : 'Размер веса'}</span>
            <span className="font-mono text-zinc-400 text-[10px]">{item.weightSize || 12}px</span>
          </div>
          <input
            type="range"
            min="10"
            max="20"
            value={item.weightSize !== undefined ? item.weightSize : 12}
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, weightSize: parseInt(e.target.value) } }))}
            className="w-full accent-white bg-zinc-950 h-1.5 rounded-lg cursor-pointer"
          />
          ...
          <div className="flex gap-2 items-center pt-1">
            <div className="flex-1">
              <label className="block text-[8px] uppercase font-bold text-zinc-500 tracking-wider mb-1">
                {lang === 'en' ? 'Weight Color' : 'Цвет веса'}
              </label>
              <div className="relative w-full h-8 rounded-lg border border-zinc-800 flex items-center justify-center bg-zinc-900 cursor-pointer hover:border-zinc-700 transition">
                <input
                  type="color"
                  value={item.weightColor || '#a1a1aa'}
                  onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, weightColor: e.target.value } }))}
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                />
                <div 
                  className="w-4 h-4 rounded border border-zinc-700 shadow-inner"
                  style={{ backgroundColor: item.weightColor || '#a1a1aa' }}
                />
              </div>
            </div>
            <button
              type="button"
              onClick={() => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, weightColor: 'rgba(255,255,255,0.4)' } }))}
              className="text-[9px] text-zinc-400 hover:text-white uppercase font-bold border border-zinc-800 h-8 px-2 rounded-lg bg-zinc-900 hover:bg-zinc-800 shrink-0 self-end"
            >
              {lang === 'en' ? 'Reset' : 'Сброс'}
            </button>
        ...
        <div className="space-y-1.5 pt-2 border-t border-zinc-800/30">
          ...
          <input
            ...
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, buttonSize: parseInt(e.target.value) } }))}
            className="w-full accent-white bg-zinc-950 h-1.5 rounded-lg cursor-pointer"
          />
        </div>
      </div>
      ...
      <div className="text-[10px] font-black uppercase text-orange-500 tracking-wider flex items-center gap-1.5">
        <span className="w-1.5 h-3.5 bg-orange-500 rounded-full" />
        <span>{lang === 'en' ? 'Dish Description' : 'ОПИСАНИЕ БЛЮДА'}</span>
      </div>
      ...
      <div className="flex gap-2 items-end">
        ...
        <div className="flex-1">
          ...
          <textarea
            ...
            onChange={(e) => updateFocusedBlock(b => ({ dishContent: { ...b.dishContent!, description: e.target.value } }))}
      ...
      <div className="text-[10px] font-black uppercase text-orange-500 tracking-wider flex items-center gap-1.5">
        <span className="w-1.5 h-3.5 bg-orange-500 rounded-full" />
        <span>{lang === 'en' ? 'Dish Photo' : 'ФОТО БЛЮДА'}</span>
      </div>
