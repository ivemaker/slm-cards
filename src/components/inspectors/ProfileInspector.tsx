...
export const ProfileInspector: React.FC<ProfileInspectorProps> = ({
  ...
  const { planType, projects, activeProjectId, updateProject } = useDev();
  ...
  const [localSubtitle, setLocalSubtitle] = React.useState(profile.subtitle || '');
  const [localBio, setLocalBio] = React.useState(profile.bio || '');

  React.useEffect(() => {
    setLocalName(focusedBlock.profileContent?.name || '');
    setLocalSubtitle(focusedBlock.profileContent?.subtitle || '');
  ...
  const handleNameBlur = () => {
    ...
    if (trimmed) {
      updateProfileContent({ name: trimmed });
      if (activeProjectId) {
  ...
  const handleBioBlur = () => {
    updateProfileContent({ bio: localBio });
  };
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (file.type === 'image/svg+xml') {
                  ...
                  reader.onload = (event) => {
                    const svgRaw = event.target?.result as string;
                    const dataUrlReader = new FileReader();
                    dataUrlReader.readAsDataURL(file);
                    dataUrlReader.onload = (duEvent) => {
                      const dataUrl = duEvent.target?.result as string;
                      updateProfileContent({
                        avatar: dataUrl,
                        avatarSvgRaw: svgRaw
                      });
                      if (activeProjectId) {
                        updateProject(activeProjectId, { avatar: dataUrl });
                      }
                  ...
                  reader.onload = async (event) => {
                    try {
                      ...
                      updateProfileContent({
                        avatar: finalUrl,
                        avatarSvgRaw: undefined
                      });
                      if (activeProjectId) {
                        updateProject(activeProjectId, { avatar: finalUrl });
                      }
                    } catch (err) {
                      console.error('Failed to upload avatar locally:', err);
                    } finally {
                      setLocalStorageUploading(false);
                    }
                  };
